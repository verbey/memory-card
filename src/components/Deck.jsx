...
function Deck() {
	...
	async function handleCardClick(event) {
		...
		const clickedCard = newCardList.find(
			(card) => card.src === clickedImgSrc
		);
		...
		if (clickedCard.clicked) {
			...
			if (currentScore + 1 > bestScore) {
				...
				bestScoreElement.classList.add(
					'animate__animated',
					'animate__tada'
				);
				bestScoreElement.addEventListener('animationend', () => {
					bestScoreElement.classList.remove(
						'animate__animated',
						'animate__tada'
					);
				});
	...
	return (
		<div
			className='deck animate__animated animate__backInRight'
			onClick={handleCardClick}
		>
			<div className='cardsContainer'>{cardComponentsList}</div>
			<div className='infoContainer'>
				<div className='rules'>
					Each time you pick a card, the deck gets shuffled. Pick a
					card that you didn&apos;t choose before to gain a point.
				</div>
